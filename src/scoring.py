...
def definition_des_arguments():
    ...
    parser.add_argument('-db', action="store", type=str,
                        ...
                        comptenant les donnees qui serviront de base de \
                        données")
...
def vecteur_PB(liste_sequences_PB):
    """
        renvoie la conversion d'une séquence de proteines block
        en liste de vesteur binaire
    """
...
def Dssp(pdb, repertoire):
    ...
    for element in glob.glob(pdb + "/*"):
        if element.endswith('.pdb'):
            os.system("./bin/dssp " + element + " -o " + repertoire +
                      "/out.txt")
...
def parse_contact(file):
    ...
    for ele in ligne:
        if ele[0].split("/")[1] not in dico:
            dico[ele[0].split("/")[1]] = {"vdw": 0, "hydrogenB": 0, "ionic": 0,
                                          "hydrophobic": 0, "polar": 0}
        else:
            if ele[4] > 0:
...
def write_csv(liste_vecteur, liste_dssp, liste_arpeggio, args, repertoir,
              file_name):
    '''
        Ecriture du fichier csv contenant les vecteurs d'environnements
    '''
    ...
    fieldnames = ['AA', 'H_ss', 'B_ss', 'E_ss', 'G_ss', 'I_ss', 'T_ss',
                  'S_ss', '-_ss', 'o_ss', 'o_ss', 'o_ss', 'o_ss', 'o_ss',
                  'o_ss', 'o_ss', 'o_ss', 'ACC', 'a_PB', 'b_PB', 'c_PB',
                  'd_PB', 'e_PB', 'f_PB', 'g_PB', 'h_PB', 'i_PB', 'j_PB',
                  'k_PB', 'l_PB', 'm_PB', 'n_PB', 'o_PB', 'p_PB', 'polar',
                  'ionic', 'hydrophobic', 'vdw', 'hydrogenB']
    ...
    for i in range(len(liste_vecteur)):
        for j in range(len(liste_vecteur[i])):
            if str(liste_vecteur[i][j][-1]) in liste_arpeggio[i].keys():
                TUPLE = (tuple(liste_dssp[j][0]) + liste_dssp[j][1] +
                         (liste_dssp[j][-1],) + tuple(liste_vecteur[i][j][0]) +
                         tuple(liste_arpeggio[i][str(liste_vecteur[i][j][-1])].values()))
            else:
                TUPLE = (tuple(liste_dssp[j][0]) + liste_dssp[j][1] +
                         (liste_dssp[j][-1],) + tuple(liste_vecteur[i][j][0]) +
                         tuple(['0', '0', '0', '0', '0']))
...
def init(path_data, args, repertoire, file_name):
    ...
    write_csv(liste_vec, liste_vec_dssp, liste_arpeggio,
              args, repertoire, file_name)
...
if __name__ == '__main__':
    ...
    # Construction du fichier csv contenant les vecteurs associés
    # à la base de données
    path_db = adress_abs + "/" + args.path_db
    init(path_db, args, repertoire, "vecteur_db.csv")
    # Construction du fichier csv contenant les vecteurs associés
    # à la séquences à prédire
    path_tp = adress_abs + "/" + args.path_to_predict
    init(path_tp, args, repertoire, "vecteur_to_predict.csv")

    # script R
    # clustering
    print("./src/clustering.R "+repertoire+"/vecteur_db.csv "+repertoire+" >"+repertoire+"clustering.txt")
    os.system("./src/clustering.R "+repertoire+"/vecteur_db.csv "+repertoire+" >"+repertoire+"clustering.txt")
    # # #Prediction
    print("./src/predict.R "+repertoire+"/vecteur_db.csv "+repertoire+"/vecteur_to_predict.csv "+repertoire+" >"+repertoire+"predict.txt")
